...
class TestLazyMethods(unittest.TestCase):
    ...
    def test_lazy_methods_in_dict(self):
        ...
        for name in self.METHOD_NAMES:
            self.assertIsInstance(AllMethods.__dict__[name], dataclasses._AutoMethod)

    def test_lazy_methods_replaced_in_dict(self):
        # Test that direct access returns a function that has been generated by
        # dataclasses and that it has been replaced in the class dict and that
        # calling the .generate() method again will return the same function
        @dataclass(frozen=True, order=True)
        class AllMethods:
            a: int

        for name in self.METHOD_NAMES:
            original = AllMethods.__dict__[name]
            ...
            self.assertIs(from_attr, from_dict)
            self.assertIs(original.generate(), from_attr)
    ...
    def test_is_dataclass_method_automethod(self):
        ...
        for name in self.METHOD_NAMES:
            ...
            self.assertIsInstance(automethod, dataclasses._AutoMethod)
    ...
    def test_callable(self):
        # Test that lazy _AutoMethod objects are callable and that calling them
        # causes them to be updated on the original class.
        @dataclass
        class AllMethods:
            a: int
            b: int

        inst1 = AllMethods(1, 1)
        inst2 = AllMethods(1, 1)
        inst3 = AllMethods(1, 2)

        eq = AllMethods.__dict__["__eq__"]
        self.assertIsInstance(eq, dataclasses._AutoMethod)

        # Test that the eq AutoMethod works as eq
        self.assertTrue(eq(inst1, inst2))
        self.assertFalse(eq(inst1, inst3))

        # Check that using it has caused it to be replaced
        eq2 = AllMethods.__dict__["__eq__"]
        self.assertIsInstance(eq2, types.FunctionType)

        # Check that this is the same object as returned by .generate
...
class TestThreading(unittest.TestCase):
    ...
    def get_slow_init_class():
        def slow_init_maker(name, cls):
            ...
            time.sleep(0.01)
            return dataclasses._init_source_maker(name, cls)

        @dataclass(init=False, repr=False, eq=False)
        class SlowInit:
            a: int

        auto_slow_init = dataclasses._AutoMethod("__init__", slow_init_maker, SlowInit)
